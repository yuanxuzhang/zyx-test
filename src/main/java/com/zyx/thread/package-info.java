 ...
 * 
 * 线程局部变量（ThreadLocal）：线程共享变量存在风险，使用ThreadLocal辅助类为各个辅助类提供各自的实力类，可以避免共享变量带来
 * 的风险。
 * 
 * stop和suspend弃用原因 【一致性问题】 【死锁问题】
 * stop:当一个线程要终止另一个线程时，无法知道什么时候调用stop方法是安全的，什么时候导致对象被破坏，临界区的代码失去原子性。
 * 在希望停止线程的时候应该中断（interrupt）线程，被中断的线程会在安全的时候停止。（做安全退出的准备）
 * suspend：supsend不会破坏对象，但是，如果用suspend挂起一个【持有一个锁】的线程，那么锁在线程恢复之前是不可用的，而如果调用
 * suspend的线程试图获得同一个锁，那么程序死锁。一个持锁等恢复，一个阻塞获得锁。
 * 
 */
